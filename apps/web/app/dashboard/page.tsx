...
export default function DashboardPage() {
  const { dashboardStats, ministrySnapshots, officialSnapshots } = useRuntimeData();
    ...
    <div className="space-y-8">
      <section className="card reveal-up overflow-hidden p-8">
        <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
          <div>
            ...
            <p className="mt-4 max-w-4xl leading-8 text-ink/75">هذه الصفحة مخصصة للعرض التنفيذي السريع أمام أصحاب المصلحة، وهي تقرأ بيانات العرض الافتراضية أو بيانات الإدارة المحفوظة محليًا عندما تكون موجودة، بما يجعل الاستعراض أقرب للواقع التشغيلي.</p>
          </div>
          <div className="rounded-[30px] bg-gradient-to-br from-ink via-teal to-gold p-5 text-white shadow-soft">
            ...
            <p className="mt-3 text-sm leading-7 text-white/80">تم تعزيز الحركة البصرية والهوية الطبقية، مع إبراز أن ما يتغير في لوحة الإدارة يمكن قراءته هنا ضمن نفس بيئة العرض.</p>
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        <div className="feature-card reveal-up lg:col-span-2">
          ...
          <p className="mt-4 leading-8 text-ink/72">من الإدارة يمكن الآن ضبط عناصر مثل حضور مجلس الوزراء، التصريحات واتساقها، التواجد في اليمن وخارجها، الأنشطة التخصصية، القرارات، التعيينات والترقيات، والحضور الرقمي للوزير.</p>
        </div>
        <div className="feature-card reveal-up">
          <h3 className="text-xl font-bold">وجهة الإدارة</h3>
          <p className="mt-3 text-sm leading-7 text-ink/72">اذهب إلى `/admin` لمراجعة عناصر التقييم وإدارة تفعيلها وأوزانها وسجلاتها الفعلية.</p>
        </div>
      </section>
